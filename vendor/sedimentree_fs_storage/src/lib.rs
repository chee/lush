...
pub struct FsStorage {
    root: PathBuf,
    /// `None` until something asks for the whole id set. Scanning `trees/` costs
    /// one `read_dir` per bucket, which is seconds on a store with tens of
...
impl FsStorage {
    ...
    /// held across the scan, so a concurrent save or delete applies to the
    /// finished set rather than racing it.
    async fn with_ids<R>(
        &self,
        f: impl FnOnce(&mut Set<SedimentreeId>) -> R,
    ) -> Result<R, FsStorageError> {
        let mut guard = self.ids_cache.lock().await;
        let ids = match guard.as_mut() {
            Some(ids) => ids,
            None => {
                let root = self.root.clone();
                let scanned =
                    tokio::task::spawn_blocking(move || Self::load_tree_ids(&root)).await?;
                guard.insert(scanned)
            }
        };
        Ok(f(ids))
    }
...
impl Storage<Sendable> for FsStorage {
    ...
    fn load_all_sedimentree_ids(
        ...
        Sendable::from_future(async move {
            ...
            self.with_ids(|ids| ids.clone()).await
        })
    }
    ...
    fn contains_sedimentree_id(
        ...
        Sendable::from_future(async move {
            ...
            self.with_ids(|ids| ids.contains(&sedimentree_id)).await
        })
    }
