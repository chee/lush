...
/// Negative registration contract: a single-item save that fails must not
/// change the set of registered tree ids (the batch twin is
/// `failed_batch_does_not_register_tree_id`).
///
/// This fork scans `trees/` lazily, so a handle cannot hold an id cache that
/// stays clean while another handle registers trees on the same root. The
/// assertion is therefore before/after equality of the registered set rather
/// than the upstream conformance helper's "unregistered beforehand".
#[tokio::test]
async fn failed_single_save_does_not_register_tree_id() -> testresult::TestResult {
    ...
    // Register via a helper instance so the on-disk commits/ dir exists, then
    // plant a roadblock file at the commit's id_dir.
    ...
    let before = Storage::<Sendable>::load_all_sedimentree_ids(&storage).await?;
    assert!(
    ...
    assert_eq!(
        before,
        Storage::<Sendable>::load_all_sedimentree_ids(&storage).await?,
        "a failed save must not change the registered tree ids"
    );
...
/// Because this fork scans `trees/` lazily, the assertion is that the failed
/// batch leaves the registered set unchanged.
#[tokio::test]
async fn failed_batch_does_not_register_tree_id() -> testresult::TestResult {
    ...
    assert_eq!(
        before,
        Storage::<Sendable>::load_all_sedimentree_ids(&storage).await?,
        "a failed batch must not change the registered tree ids"
    );
...
#[tokio::test]
async fn blob_listing_covers_every_blob_and_slices_are_bounded() -> testresult::TestResult {
    let dir = tempfile::tempdir()?;
    let storage = FsStorage::new(dir.path().to_path_buf())?;
    let signer = test_signer();
    let id = make_sedimentree_id(0x7D);
    let large: Vec<u8> = (0..1_100_000).map(|index| (index % 251) as u8).collect();
    let blobs = [vec![0x11; 17], large.clone(), vec![0x33; 29]];
    for (index, blob) in blobs.into_iter().enumerate() {
        let verified = VerifiedMeta::<LooseCommit>::seal::<Sendable, _>(
            &signer,
            (id, CommitId::new([(index + 1) as u8; 32]), BTreeSet::new()),
            VerifiedBlobMeta::new(Blob::new(blob)),
        )
        .await;
        Storage::<Sendable>::save_loose_commit(&storage, id, verified).await?;
    ...
    let entries = storage.list_blobs(id).await?;
    assert_eq!(entries.len(), 3);
    let mut byte_lens: Vec<u64> = entries.iter().map(|entry| entry.byte_len).collect();
    byte_lens.sort_unstable();
    assert_eq!(byte_lens, vec![17, 29, large.len() as u64]);
    let repeated: BTreeSet<String> = storage
        .list_blobs(id)
        .await?
        .into_iter()
    ...
    let entry = entries
        .into_iter()
        .find(|entry| entry.byte_len == large.len() as u64)
        .unwrap();
    let mut loaded = Vec::new();
    let mut offset = 0;
    while offset < entry.byte_len {
        let slice = storage
            .load_blob_slice(id, entry.cursor.clone(), offset, 128 * 1024)
            .await?;
        assert!(!slice.is_empty());
        assert!(slice.len() <= 128 * 1024);
    ...
    assert!(
        storage
            .load_blob_slice(id, entry.cursor.clone(), entry.byte_len, 128 * 1024)
            .await?
            .is_empty()
    );
    assert!(
        storage
            .load_blob_slice(id, "invalid".into(), 0, 128 * 1024)
            .await
            .is_err()
    );
    assert!(
        storage
            .load_blob_slice(id, "2/aa/bb".into(), 0, 128 * 1024)
            .await
            .is_err()
    );
    assert!(
        storage
            .load_blob_slice(id, entry.cursor, entry.byte_len + 1, 128 * 1024)
            .await
            .is_err()
    );
    ...
    assert!(
        storage
            .list_blobs(make_sedimentree_id(0x7E))
            .await?
            .is_empty()
    );
