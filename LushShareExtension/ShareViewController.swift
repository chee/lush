...
class ShareViewController: NSViewController {
    ...
    override func viewDidAppear() {
        ...
        Task {
            do {
                let handoff = try ShareIntake.prepareHandoffDirectory()
                let items = try await ShareIntake.loadItems(from: context, into: handoff.directory)
...
private enum ShareIntake {
    static let appGroupIdentifier = "group.party.chee.patchwork.lush"

    static func prepareHandoffDirectory() throws -> (id: String, directory: URL) {
        guard let container = FileManager.default.containerURL(
            forSecurityApplicationGroupIdentifier: appGroupIdentifier
        ) else {
            throw ShareIntakeError.missingContainer
        }
...
private enum ShareIntakeError: LocalizedError {
