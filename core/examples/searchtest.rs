...
fn main() -> anyhow::Result<()> {
    ...
    let core = Core::new("/tmp/searchtest-data".into(), None).map_err(|e| anyhow::anyhow!("{e}"))?;
    let root = core.ensure_folder(None).map_err(|e| anyhow::anyhow!("{e}"))?;
    let note1 = core.create_note("surface note".into()).map_err(|e| anyhow::anyhow!("{e}"))?;
    ...
    let sub = core.create_subfolder("deeper".into()).map_err(|e| anyhow::anyhow!("{e}"))?;
    core.ensure_folder(Some(sub.clone())).map_err(|e| anyhow::anyhow!("{e}"))?;
    let note2 = core.create_note("hidden note".into()).map_err(|e| anyhow::anyhow!("{e}"))?;
    core.update_note_spans(
        note2,
    ...
    core.ensure_folder(Some(root.clone())).map_err(|e| anyhow::anyhow!("{e}"))?;
    ...
    println!("root entries: {:?}", entries.iter().map(|e| format!("{}({})", e.name, e.kind)).collect::<Vec<_>>());
