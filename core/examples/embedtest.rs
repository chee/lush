...
async fn main() -> anyhow::Result<()> {
    ...
    let repo = Repo::start("/tmp/embedtest-data".into(), richtext_core::repo::DEFAULT_SERVER.into()).await?;
    repo.wait_connected(Duration::from_secs(15)).await;

    let asset = repo.create_doc(|doc| shapes::init_file_doc(doc, "dot.png", "png", "image/png", png.clone())).await?;
    let note = repo.create_doc(|doc| shapes::init_rich_note(doc, "embed test")).await?;
    ...
    let fresh = Repo::start("/tmp/embedtest-fresh".into(), richtext_core::repo::DEFAULT_SERVER.into()).await?;
    fresh.ensure_doc(note).await?;
    fresh.ensure_doc(asset).await?;
    assert!(fresh.wait_for_doc(note, Duration::from_secs(20)).await, "note arrived");
    assert!(fresh.wait_for_doc(asset, Duration::from_secs(20)).await, "asset arrived");
    let spans = fresh.read_doc(note, |d| shapes::spans_to_json(d)).await?;
    println!("note spans: {}", serde_json::to_string(&spans)?);
    let bytes = fresh.read_doc(asset, |d| Ok(shapes::file_bytes(d))).await?.unwrap();
